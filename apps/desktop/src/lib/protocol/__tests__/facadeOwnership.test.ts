...
describe("facade ownership — Ticket 2 invariants", () => {
  ...
  it("has no persistent layers field — only snapshot cache", () => {
  ...
  it("transform lifecycle: transient does not mutate snapshot, commit does via delta", () => {
    const f = new EditorFacade();
    f.addLayer("A");
  ...
  it("pointermove never calls adapter/IPC (spy)", () => {
    const f = new EditorFacade();
    f.addLayer("A");
    const id = f.snapshot.layers[0].id;
  ...
  it("brush: activeStroke lives in transient, stroke command only on pointerup", () => {
    const f = new EditorFacade();
    f.addLayer("L");
    const id = f.snapshot.layers[0].id;
    const spy = vi.spyOn(bridge, "applyCommand");
  ...
  it("add/delete/opacity are single-command, no optimistic persistent mutation on failure", () => {
    const f = new EditorFacade();
    ...
    expect(() => bridge.applyCommand({ contractVersion: 999, command: { type: "addLayer", name: "bad" } } as never)).toThrow();
    expect(f.snapshot.layers.length).toBe(0); // still 0, no optimistic write
  });

  it("undo/redo via delta, version monotonic, no TS history mirror", () => {
    const f = new EditorFacade();
    f.addLayer("A");
    const id = f.snapshot.layers[0].id;
    f.setOpacity(id, 0.3);
  ...
  it("version checks: stale delta does not overwrite newer snapshot", () => {
    const f = new EditorFacade();
    f.addLayer("A"); // v1
    f.addLayer("B"); // v2
    expect(f.renderedVersion).toBe(2);
  ...
  it("migration invariant: after migration, exactly one persistent owner per op", () => {
    const f = new EditorFacade();
