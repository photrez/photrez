...
describe("H0 stream — Rust/engine semantics", () => {
  it("native append + seq uniqueness + cursor movement", () => {
    const r1 = bridge.applyCommand({ contractVersion: 1, command: { type: "addLayer", name: "A" } });
    const r2 = bridge.applyCommand({ contractVersion: 1, command: { type: "addLayer", name: "B" } });
    void r2;
    let q = bridge.getHistoryQuery();
    ...
    const u = bridge.applyCommand({ contractVersion: 1, command: { type: "undo" } });
    expect(u.status).toBeUndefined(); // native step applied directly
    q = bridge.getHistoryQuery();
    expect(q.cursor).toBe(1);
    const rd = bridge.applyCommand({ contractVersion: 1, command: { type: "redo" } });
    expect(rd.status).toBeUndefined();
    expect(bridge.getHistoryQuery().cursor).toBe(2);
  });

  it("DV independent from cursor: no-op undo bumps DV, cursor stays", () => {
    bridge.applyCommand({ contractVersion: 1, command: { type: "addLayer", name: "A" } });
    const before = bridge.applyCommand({ contractVersion: 1, command: { type: "noop" } });
    void before;
    const dv = JSON.parse(JSON.stringify(bridge.getHistoryQuery()));
    const r = bridge.applyCommand({ contractVersion: 1, command: { type: "undo" }, expectedVersion: undefined });
  ...
  it("redo region truncated by new forward command; seq never reused", () => {
    bridge.applyCommand({ contractVersion: 1, command: { type: "addLayer", name: "A" } });
    bridge.applyCommand({ contractVersion: 1, command: { type: "undo" } });
    bridge.applyCommand({ contractVersion: 1, command: { type: "addLayer", name: "B" } });
    const q = bridge.getHistoryQuery();
  ...
  it("external append validates adapter and advances DV exactly once", () => {
    const dv0 = bridge.applyCommand({ contractVersion: 1, command: { type: "noop" } }).documentVersion;
    ...
    ).toThrow(/E_UNKNOWN_ADAPTER/);
    const r = bridge.applyCommand({
      contractVersion: 1,
  ...
  it("stale expectedVersion rejected on stream commands", () => {
    expect(() =>
      bridge.applyCommand({
        contractVersion: 1,
        expectedVersion: 999,
  ...
  it("walker hands off external entries: status=external, no DV bump until cursor commit", () => {
    const facade = getFacade("docH0");
    facade.addLayer("L"); // v1
    const dv = facade.renderedVersion;
    const r = bridge.applyCommand({ contractVersion: 1, expectedVersion: dv, command: { type: "undo" } });
    if (r.status === "external") {
      expect(r.externalSeq).toBeGreaterThan(0);
      expect(r.documentVersion).toBe(dv); // handoff does NOT bump
      const c = bridge.historyCursorCommit(r.externalSeq!, "undo");
      expect(c.documentVersion).toBe(dv + 1); // exactly one bump at confirm
      expect(bridge.getHistoryQuery().cursor).toBe(0);
    } else {
...
describe("H0 shim — unrecorded transition detection & degraded projection", () => {
  it("failed RecordExternalTransition sets history-degraded with markers (never silent)", () => {
...
describe("H0 final invariants", () => {
  it("INV1: pending external barrier - mutating/undo commands rejected with E_EXTERNAL_PENDING until cursor_commit succeeds", () => {
    bridge.registerPayloadAdapter("ts-external");
    bridge.applyCommand({
      contractVersion: 1,
      command: { type: "recordExternalTransition", label: "legacy op", affectedLayerIds: [], adapterId: "ts-external", token: "t1", memoryCostBytes: 0 },
    });
    const hand = bridge.applyCommand({ contractVersion: 1, command: { type: "undo" } });
    ...
    ).toThrow(/E_EXTERNAL_PENDING/);
    // another history command during pending window
    expect(() => bridge.applyCommand({ contractVersion: 1, command: { type: "undo" } })).toThrow(
      /E_EXTERNAL_PENDING/,
    );

    // barrier exposed via query
    expect(bridge.getHistoryQuery().pendingExternal?.seq).toBe(hand.externalSeq);

    // mismatched commit rejected; barrier retained
    expect(() => bridge.historyCursorCommit(99, "undo")).toThrow(/E_CURSOR_MISMATCH/);
    expect(bridge.getHistoryQuery().pendingExternal?.seq).toBe(hand.externalSeq);

    // correct commit clears the barrier - forward command now accepted
    bridge.historyCursorCommit(hand.externalSeq!, "undo");
    expect(bridge.getHistoryQuery().pendingExternal ?? null).toBeNull();
    const okCmd = bridge.applyCommand({ contractVersion: 1, command: { type: "addLayer", name: "Y" } });
    expect(okCmd.documentVersion).toBeGreaterThan(0);
  });

  it("INV2: authoritative DV sync - legacy mutation -> record -> facade synced -> next expectedVersion accepted", () => {
    // legacy mutation happens OUTSIDE the protocol (TS state only)
    const res = recordExternalTransitionFor("docSync", {
      label: "Move Layer",
      affectedLayerIds: ["bg"],
      snapshot: { layers: [] },
    });
    ...
    const proj = getHistoryProjection("docSync");
    ...
    const next = bridge.applyCommand({
      contractVersion: 1,
      expectedVersion: N,
      command: { type: "noop" },
    });
  ...
  it("INV3: cursor commit failure -> historyDegraded=true and deterministic no-op afterwards", () => {
    bridge.registerPayloadAdapter("ts-external");
    bridge.applyCommand({
      contractVersion: 1,
    ...
    const c = confirmExternalCursor("docH0b", hand.externalSeq!, "undo");
    ...
    const r2 = recordExternalTransitionFor("docH0b", { label: "x", affectedLayerIds: [], snapshot: null });
    const c2 = confirmExternalCursor("docH0b", hand.externalSeq!, "undo");
    expect(r2.ok).toBe(false);
    expect(c2.ok).toBe(false);
    expect(recSpy).not.toHaveBeenCalled();
    expect(getHistoryProjection("docH0b").degraded).toBe(true);
    expect(getHistoryProjection("docH0b").degradeReason).toBe("CURSOR_COMMIT_FAILED");
  });
});
