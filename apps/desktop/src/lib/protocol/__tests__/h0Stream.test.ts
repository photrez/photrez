...
describe("H0 stream — Rust/engine semantics", () => {
  it("native append + seq uniqueness + cursor movement", async () => {
    const r1 = await bridge.applyCommand({ contractVersion: CONTRACT_VERSION, command: { type: "addLayer", name: "A", id: "A-id", width: 100, height: 100, index: 0 } });
    const r2 = await bridge.applyCommand({ contractVersion: CONTRACT_VERSION, command: { type: "addLayer", name: "B", id: "B-id", width: 100, height: 100, index: 0 } });
    void r2;
    ...
    const u = await bridge.applyCommand({ contractVersion: CONTRACT_VERSION, command: { type: "undo" } });
    expect(u.status).toBeUndefined(); // native step applied directly
    q = await bridge.getHistoryQuery();
    expect(q.cursor).toBe(1);
    const rd = await bridge.applyCommand({ contractVersion: CONTRACT_VERSION, command: { type: "redo" } });
  ...
  it("DV independent from cursor: no-op undo bumps DV, cursor stays", async () => {
    await bridge.applyCommand({ contractVersion: CONTRACT_VERSION, command: { type: "addLayer", name: "A", id: "A-id", width: 100, height: 100, index: 0 } });
    const before = await bridge.applyCommand({ contractVersion: CONTRACT_VERSION, command: { type: "noop" } });
    void before;
    const dv = JSON.parse(JSON.stringify(await bridge.getHistoryQuery()));
    const r = await bridge.applyCommand({ contractVersion: CONTRACT_VERSION, command: { type: "undo" }, expectedVersion: undefined });
  ...
  it("redo region truncated by new forward command; seq never reused", async () => {
    await bridge.applyCommand({ contractVersion: CONTRACT_VERSION, command: { type: "addLayer", name: "A", id: "A-id", width: 100, height: 100, index: 0 } });
    await bridge.applyCommand({ contractVersion: CONTRACT_VERSION, command: { type: "undo" } });
    await bridge.applyCommand({ contractVersion: CONTRACT_VERSION, command: { type: "addLayer", name: "B", id: "B-id", width: 100, height: 100, index: 0 } });
    const q = await bridge.getHistoryQuery();
  ...
  it("external append validates adapter and advances DV exactly once", async () => {
    const dv0 = (await bridge.applyCommand({ contractVersion: CONTRACT_VERSION, command: { type: "noop" } })).documentVersion;
  ...
  it("walker hands off external entries: status=external, no DV bump until cursor commit", async () => {
    ...
    const r = await bridge.applyCommand({ contractVersion: CONTRACT_VERSION, expectedVersion: dv, command: { type: "undo" } });
...
describe("H0 final invariants", () => {
  it("INV1: pending external barrier - mutating/undo commands rejected with E_EXTERNAL_PENDING until cursor_commit succeeds", async () => {
    ...
    const hand = await bridge.applyCommand({ contractVersion: CONTRACT_VERSION, command: { type: "undo" } });
    expect(hand.status).toBe("external");
    expect(hand.externalSeq).toBeGreaterThan(0);

    // mutating command during pending window
    await expect(
      bridge.applyCommand({ contractVersion: CONTRACT_VERSION, command: { type: "addLayer", name: "X", id: "X-id", width: 100, height: 100, index: 0 } }),
    ).rejects.toThrow(/E_EXTERNAL_PENDING/);
    // another history command during pending window
    await expect(
      bridge.applyCommand({ contractVersion: CONTRACT_VERSION, command: { type: "undo" } }),
    ).rejects.toThrow(/E_EXTERNAL_PENDING/);
    ...
    const okCmd = await bridge.applyCommand({ contractVersion: CONTRACT_VERSION, command: { type: "addLayer", name: "Y", id: "Y-id", width: 100, height: 100, index: 0 } });
