...
fn main() {
    ...
    if target.contains("windows") {
        // Copy WebView2Loader.dll BEFORE tauri_build::build(): tauri-build
        // validates bundle.resources (tauri.conf.json references
    ...
    // Custom manifest compiler workaround for MinGW/GNU target to prevent COMCTL32 version mismatch
    if target.contains("windows-gnu") {
...
/// Copy WebView2Loader.dll from the webview2-com-sys build output into the
/// profile directory (target/{debug,release}/) so tauri.conf.json's
/// bundle.resources entry resolves. Must run BEFORE tauri_build::build().
